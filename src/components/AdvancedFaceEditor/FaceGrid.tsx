...
const FaceGrid: React.FC<FaceGridProps> = ({
  ...
  React.useEffect(() => {
    ...
    if (faceImages.length > 0) {
      console.log('First face filename:', faceImages[0].filename)
  ...
  const renderFaceThumbnail = (face: FaceImage) => {
    ...
    const imageUrl = face.thumbnailUrl || getImageUrl(face.filename)
    ...
    return (
      ...
      >
        ...
        <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-md overflow-hidden relative" style={{ height: size }}>
          {face.thumbnailUrl || face.filePath ? (
            <img
              src={face.thumbnailUrl || getImageUrl(face.filename)}
              alt={face.filename}
          ...
          <div 
            ...
            style={{ display: face.thumbnailUrl || face.filePath ? 'none' : 'flex' }}
          >
            {face.filename}
          </div>
